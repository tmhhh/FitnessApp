    ...
    getById: async (req, res) => {
        try {
            ...
            const relatedProducts = await productModel
                .find({'prodCategory.cateFilter.filterName': product?.prodCategory?.cateFilter?.filterName})
                .limit(3);
            return res
                .status(200)
                .json({isSuccess: true, data: {product, relatedProducts}});
        } catch (e) {
            return res
                .status(500)
                .json({isSuccess: false, error: "Internal Server Error " + e});
        }
    },
